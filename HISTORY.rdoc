...
Previous version utilized a system of "commit tagging" to identify types of commits. This proved less thant optimal --it was unconventional but worse it was easy to forget to put the proper label in the message. The new version of VClog use a customizable heuristics systems instead. Rather then be limited to a strict syntax structure, you can write matching rules in .config/vclog/rules.rb that determine thte commit type, and set descriptive labels for each type. This makesit very easy to get excellent History output.
